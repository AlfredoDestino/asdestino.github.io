# Alfredo Destino Portfolio

Computer Engineer | Lead Developer — Enterprise Systems — C# Developer | Information Security Enthusiast

## About Me
Computer Engineer and Lead Software Developer with 7 years of experience designing and deploying secure enterprise systems for national institutions in Angola, including Sonangol EP, Ministry of Interior and the Tax Authority (AGT). 
Awarded Best Student in Programming & Networks (2017-2019) at Kimpa Vita University, Uige, Angola; where served as Lab monitor and Lecturer.
- Seeking MSc in Cyber Security in Switzerland. LinkedIn: [www.linkedin.com/in/asdestino](www.linkedin.com/in/asdestino).

## Projects

### SONA-SSES (Enterprise Security System, Sonangol EP, 2025–Present)
- Phase 1: Secure system for 200+ users with RBAC, audit log, encrypted data (C#/.NET + MySQL).  
- Phase 2: Extend to remote access via VPN/ZTNA (Master's objective).  
- [User Manual (Levels 2 & 3)](SONA_SSES_STD23_ENG_MANUAL.pdf)  
- [Promotional Video](https://youtu.be/41C7MBI42T4) (private – request access)

### NGOLO (Criminal Monitoring System, Ministry of Interior, 2023–2024)
- Phase 1: Real-time platform for provincial law enforcement, reducing response time by 40% for 50+ units.  
- Phase 2: Secure remote access for officials abroad via VPN/ZTNA.  
- [System Presentation](APRES_NGOLO_V19_ENG.pdf)  
- Full demo available upon request (classified).

### EMPACTA (Commercial & Billing System, 2022)
- AGT-validated ERP for SMEs, processing 1,000+ invoices/month.  
- [Validation Certificate](EMPACTA_Cert_388_2022.pdf)

## Skills
- Programming: C#, SQL, .NET/WPF...  
- Security: VPN, ZTNA, RBAC, Encryption...  
- Tools: Visual Studio, Git, MySQL...  

Contact: alfredodestinn@gmail.com | +244-945-380-783 | Wapp: +244-923-759-411
